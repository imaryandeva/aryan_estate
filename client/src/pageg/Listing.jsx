...
export default function Listing() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(false);
        const fetchListing = await fetch(
          `/api/listing/get/${params.listingId}`
        );
        const data = await fetchListing.json(); // Corrected: Use .json() method
        setLoading(false);
  ...
  return (
    <main>
      ...
      {error && (
        <p className="text-center my-7 text-3xl">
      ...
      {listing && !loading && !error && (
        <div>
          ...
          <div className="flex flex-col max-w-4xl mx-auto gap-6 p-3 my-7">
